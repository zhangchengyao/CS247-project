...
class Resonation(nn.Module):
    ...
    def forward(self, input):
        ...
        return torch.log(input + torch.mul(input, self.softmax(self.w).t()[ind]))

class Resonation2(nn.Module):
    def __init__(self, src_dim, k):
        super(Resonation2, self).__init__()
        self.w = nn.Parameter(torch.zeros((src_dim,k),device='cuda').uniform_(0,1))
        self.softmax = nn.Softmax(dim=1)
    ...
    def forward(self, input):
        ...
        if torch.cuda.is_available():
            W = W.cuda()
        softmax_w = self.softmax(self.w)
        val, ind = torch.max(torch.mm(input.view(-1,src_dim), softmax_w), dim = 1)
        val = val.view(batch_size,-1)
        ind = ind.view(batch_size,-1)
        for i in range(batch_size):
            for j in range(1,trg_seq_len):
                W[i,j] = (softmax_w).t()[ind[i][j-1]]
        
        return torch.log(input + torch.mul(input, W))
    ...
    def reinforce(self, input):
        ...
        if not self.setmemory:
            self.setmemory = True
            val, ind = torch.max(torch.mm(input, softmax_w), dim = 1)
            self.memory =  (softmax_w).t()[ind]
        else:
...
class PointerGen(nn.Module):
    def __init__(self, h_dim, s_dim, x_dim):
        super(PointerGen, self).__init__()
        self.w_h = nn.Linear(h_dim, 1, bias=False)
        self.w_s = nn.Linear(s_dim, 1, bias=False)
        self.w_x = nn.Linear(x_dim, 1, bias=False)
        self.bias = nn.Parameter(torch.zeros(1))
    def forward(self, h, s, x):
        return torch.sigmoid(self.w_h(h) + self.w_s(s) + self.w_x(x) + self.bias).contiguous()
...
class Seq2SeqLSTMAttention(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.ref = opt.reinforce
        self.pointer = opt.pointer
        ...
        '''
        self.vocabref = torch.nn.Sequential(
          torch.nn.Linear(self.vocab_size, self.trg_hidden_dim),
          torch.nn.Linear(self.trg_hidden_dim, self.vocab_size),
        )
        '''
        if self.pointer:
            self.pointergen = PointerGen(self.trg_hidden_dim, self.src_hidden_dim, self.emb_dim).cuda()
            self.pgen = None 
        # copy attention
    ...
    def decode(self, trg_inputs, src_map, oov_list, enc_context, enc_hidden, trg_mask, ctx_mask):
        ...
        :param
                ...
                src_map  :         (batch_size, ), almost the same with src but oov words are replaced with temporary oov index, for copy mechanism to map the probs of pointed words to vocab words. The word index can be beyond vocab_size, e.g. 50000, 50001, 50002 etc, depends on how many oov words appear in the source text
        ...
        if not do_word_wisely_training:
            ...
            if self.pointer:
                self.pgen = self.pointergen(h_tildes.permute(1, 0, 2), decoder_outputs, trg_emb).permute(1, 0 ,2)
            ...
            if self.ref:
                decoder_logits = self.vocabref(decoder_logits.view(-1, self.vocab_size)).view(batch_size, max_length, -1)
                #decoder_logits = self.vocabref(torch.exp(decoder_logits))
            '''
            (3) Copy Attention
            '''
            if self.copy_attention:
    ...
    def merge_copy_probs(self, decoder_logits, copy_logits, src_map, oov_list):
        ...
        if self.pointer:
            #print(self.pgen.size())
            #print(decoder_logits.view(batch_size, max_length, -1).size())
            flattened_decoder_logits = self.pgen.contiguous().view(batch_size *max_length, -1) * decoder_logits.view(batch_size * max_length, self.vocab_size)
        else:
        ...
        if self.pointer:
            flattened_decoder_logits = flattened_decoder_logits.scatter_add_(1, expanded_src_map, (1-self.pgen.contiguous().view(batch_size*max_length, -1))*copy_logits.view(batch_size * max_length, -1))
        else:
    ...
    def generate(self, trg_input, dec_hidden, enc_context, ctx_mask=None, src_map=None, oov_list=None, max_len=1, return_attention=False):
        ...
        for i in range(max_len):
            ...
            h_tilde, attn_weight, attn_logit = self.attention_layer(decoder_output.permute(1, 0, 2), enc_context, encoder_mask=ctx_mask)
            if self.pointer:
                #print(h_tilde.size(),decoder_output.permute(1,0,2).size(), dec_input.size())
                self.pgen = self.pointergen(h_tilde, decoder_output.permute(1, 0, 2), dec_input.permute(1,0,2))
            
            # compute the output decode_logit and read-out as probs: p_x = Softmax(W_s * h_tilde)
            # (batch_size, trg_len, trg_hidden_size) -> (batch_size, 1, vocab_size)
            decoder_logit = self.decoder2vocab(h_tilde.view(-1, trg_hidden_dim))
            if self.ref:
                decoder_logit = self.vocabref(decoder_logit.view(-1,self.vocab_size))
                #decoder_logit = self.vocabref.reinforce(torch.exp(decoder_logit))
    ...
    def decode_without_copy(self, trg_inputs, enc_context, enc_hidden, trg_mask, ctx_mask):
        ...
        if self.do_teacher_forcing():
            ...
            if self.ref:
                decoder_logits += torch.nn.functional.log_softmax(self.vocabref(torch.exp(decoder_logits.view(-1, self.vocab_size))))
                #decoder_logits = self.vocabref(torch.exp(decoder_logits))
            ...
            for di in range(max_length):
                ...
                decoder_logit = self.decoder2vocab(h_tilde.view(-1, trg_hidden_dim))
                if self.ref:
                    decoder_logit += torch.nn.functional.log_softmax(self.vocabref(torch.exp(decoder_logit.view(-1,  self.vocab_size))))
                    #decoder_logit = self.vocabref(torch.exp(decoder_logit))
