...
python -m train -data_path_prefix "data/$DATA_NAME/$DATA_NAME" -vocab_path "data/$DATA_NAME/$DATA_NAME.vocab.pt" -exp "$DATA_NAME" -exp_path "$ROOT_PATH/exp/$EXP_NAME/%s.%s" -batch_size 32 -run_valid_every 1000 -save_model_every 1000 -bidirectional -pointer -copy_attention -attention_mode "$ATTENTION" -copy_mode "$ATTENTION"  -beam_size 16 -beam_search_batch_size 8 -train_ml -epoch 20 -must_teacher_forcing
$SHELL